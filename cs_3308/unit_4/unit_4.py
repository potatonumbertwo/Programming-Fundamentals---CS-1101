...
def parseToken(line):
    ...
    lineToTokens = splitChars(line)

    # for each token in the line process
    for elmt in lineToTokens:
        ...
        # This value will provide the total size of the corpus
        # in terms of the number of terms in the entire collection
        tokens += 1
        ...
        if not (lowerElmt in database.keys()):
            ...
            database[lowerElmt].documentsId = dict()
            database[lowerElmt].docs = 0

            # if the document is not currently in the postings
            # list for the term then add it
        if not (documents in database[lowerElmt].documentsId.keys()):
            database[lowerElmt].docs += 1
            database[lowerElmt].documentsId[documents] = 0

            # Increment the counter that tracks the term frequency
        database[lowerElmt].documentsId[documents] += 1

    return lineToTokens


#
# Open and read the file line by line, parsing for tokens and processing.
# All of the tokenizing is done in the parsetoken() function.
# You should design your indexer program keeping the tokenizing
# as a separate function that will process a string
# as you will be able to reuse code for future assignments
#
...
if __name__ == "__main__":
    ...
    #
    folder = "/Users/crowfordcindy/Desktop/git/cs_1101/cs_3308/unit_4/cacm"
    ...
    con = sqlite3.connect("/Users/crowfordcindy/Desktop/git/cs_1101/cs_3308/unit_4/database")
    ...
    cur.execute(
        "create table if not exists Posting (TermId int, DocId int, tfidf real, docfreq int, termFrequence int)")
